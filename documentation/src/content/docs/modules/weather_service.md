...
- This is the project's first real use of a PostGIS `geometry` column (the flight-log services deliberately stuck to plain `double precision` lat/lon since they had no spatial-query need yet). Weather zones are the opposite: spatial containment (`ST_Contains`, for a point) and intersection (`ST_Intersects`, for an extent) are the core query need — see the `.../current` endpoints below — hence the `geometry` type and its GIST index from the start.
...
| `GET` | `/api/v1/visibility-zones/observed/current` | Zone(s) whose observed polygon contains a point (`lat`, `lon`) or intersects an extent (`lat1`/`lon1`/`lat2`/`lon2`), latest or as of `at` |
...
| `GET` | `/api/v1/visibility-zones/forecast/current` | Zone(s) whose forecast polygon (for a required `at`) contains a point or intersects an extent |
...
`POST .../forecast-reports` takes the identical body shape (it's a different endpoint, not a different field, that marks it as a forecast). `ceilingFt` is required when `state` is `"foggy"` and rejected otherwise, mirroring the table's check constraint. The `/wind-zones/...` equivalents drop `ceilingFt` and draw `state` from the wind enum instead. Every ingest endpoint inserts via `ON CONFLICT (recorded_at, report_id) DO NOTHING` and returns `201` with only the reports that were newly inserted — an idempotent retry that resubmits already-stored reports gets `201` back with an empty (or partial) array rather than an error, same convention as the flight-log services' ingest endpoints. There's no zone registry to validate against, so ingestion never `404`s. Every `polygon` vertex's `[lon, lat]` pair is range-checked against the same bounds as query-param coordinates — see **Coordinate ranges** below.

**Coordinate ranges**: every latitude value this service accepts — `lat`, `lat1`, `lat2` in a query string, or the second element of any `[lon, lat]` polygon vertex — must satisfy `-90 < lat < 90`, exclusive of the poles themselves (a latitude of exactly ±90 makes longitude degenerate, since every meridian meets at the pole). Every longitude value — `lon`, `lon1`, `lon2`, or a vertex's first element — must satisfy `-180 <= lon <= 180`, inclusive of the antimeridian. An extent (`lat1`/`lon1`/`lat2`/`lon2`) must additionally have nonzero width and height: `lat1` and `lat2` must differ, and `lon1` and `lon2` must differ, regardless of which corner is which (see **Point vs. extent** below on corner ordering). Any violation is a `400`.

`GET /visibility-zones/observed` returns one entry per zone (its most recent observed report by default) — the "what does the sky look like right now, area-wide" view a map/dashboard would render; pass `?at=<timestamp>` to instead get each zone's most recent observed report **as of** that time (last known state at-or-before `at`). `GET /visibility-zones/observed/current` (optionally with `at`) returns whichever zone(s)' observed polygon matches a spatial filter, given as **either** a point (`lat`, `lon` — polygon must *contain* the point) **or** an extent (`lat1`, `lon1`, `lat2`, `lon2` — polygon must *intersect* the extent), never both.

**Point vs. extent**: `lat`/`lon` and `lat1`/`lon1`/`lat2`/`lon2` are mutually exclusive — supplying any field from both groups, supplying only part of either group (e.g. `lat` without `lon`, or `lat1`/`lon1` without `lat2`/`lon2`), or supplying neither group at all, is a `400`; exactly one complete group is required. `lat1`/`lon1` and `lat2`/`lon2` are any two diagonal corners of the extent — they don't need to be given in a particular order (e.g. southwest-then-northeast); the query normalizes them into a `min`/`max` bounding box itself (`ST_MakeEnvelope` with `LEAST`/`GREATEST` on each axis) before testing intersection. This doesn't handle an extent that crosses the antimeridian (±180° longitude) — see [Open questions](#open-questions).
...
`GET /visibility-zones/forecast` and `.../forecast/current` require `at` (there's no default) and return, per zone, the forecast whose `recordedAt` is closest to `at`; if a zone has multiple forecasts targeting that same `recordedAt` (re-forecasts issued at different times), the most recently issued (`ingestedAt`) wins. `.../forecast/current` takes the same point-or-extent spatial filter, with the same mutual-exclusivity rules, as `.../observed/current` above — just tested against the resolved forecast's polygon instead of the observed one.
...
- The `.../current` extent (`lat1`/`lon1`/`lat2`/`lon2`) doesn't handle boxes crossing the antimeridian (±180° longitude) — it's a plain `min`/`max` bounding box, so a box actually intended to wrap around the date line would silently search the wrong side of the globe instead (nonzero-width/height validation doesn't catch this, since such a box isn't degenerate — just misinterpreted). Revisit if a consumer needs that.
