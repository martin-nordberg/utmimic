...
- The `.../current` extent (`lat1`/`lon1`/`lat2`/`lon2`) doesn't handle boxes crossing the antimeridian (±180° longitude) — it's a plain `min`/`max` bounding box, so a box actually intended to wrap around the date line would silently search the wrong side of the globe instead. Revisit if a consumer needs that.
- The extent fields aren't range-validated either (same as `lat`/`lon` above).
