...
- `src/drone-registrations-client.ts` — thin `fetch` wrapper around [Drone Registrations Service](/modules/drone_registrations_service/)'s API (`config.DRONE_REGISTRATIONS_SERVICE_URL`):
  - `DroneRegistrationsServiceUnavailableError` — thrown when the `fetch` call itself rejects (network error, including a timeout — see below) or the response status is `>= 500`. See the resolved decision above for why this maps to `503` rather than a fallback.
  - A private `get(path)` helper does the `fetch` call — with `signal: AbortSignal.timeout(5000)` — and classifies network errors, timeouts, and `5xx` into `DroneRegistrationsServiceUnavailableError`; each exported function calls it and then does its own `200`/`404`/etc. interpretation on top, since the four calls have different success shapes (boolean existence vs. an extracted field). The timeout was added after Phase 6's manual end-to-end verification found that, in this environment, a connection attempt to a stopped Drone Registrations Service instance hangs indefinitely rather than rejecting with `ECONNREFUSED` — without a bound, that would hang this service's own request handling instead of returning `503`.
