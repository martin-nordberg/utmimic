...
// `defaultHook` is a per-instance constructor option, and this service creates
// one OpenAPIHono instance per route module (plus the top-level app) rather
// than a single instance — use this everywhere instead of `new OpenAPIHono()`
// so the override above is guaranteed on every instance regardless of how
// `.route()` mounting happens to propagate it.
