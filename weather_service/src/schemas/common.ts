...
const spatialFilterFields = {
  lat: z.coerce.number().optional().openapi({ example: 47.62 }),
  lon: z.coerce.number().optional().openapi({ example: -122.35 }),
  lat1: z.coerce.number().optional().openapi({ example: 47.55 }),
  lon1: z.coerce.number().optional().openapi({ example: -122.45 }),
  lat2: z.coerce.number().optional().openapi({ example: 47.7 }),
  lon2: z.coerce.number().optional().openapi({ example: -122.25 }),
};
 ...
 * partial group. Domain-agnostic (no visibility/wind-specific fields), so it's shared here
 * rather than duplicated per zone kind, unlike the zone-specific report schemas.
 */
